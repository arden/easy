...
import (
	"context"
	"fmt"
	"github.com/arden/easy/mq/nsq/internal/color"
	"github.com/arden/easy/mq/nsq/logger"
	"log"
	"runtime"
	"time"
	...
	var fn gonsq.HandlerFunc = func(m *gonsq.Message) error {
		...
		if err != nil {
			...
			if _, ok := err.(*debugError); ok {
				h.Logger.Debug(errdata)
				h.Logger.Debugf("[NSQ Consumer Error] "+h.getChannelTopic()+" %s", err.Error())
			} else {
				h.Logger.Debug(errdata)
				h.Logger.Errorf("[NSQ Consumer Error] "+h.getChannelTopic()+" %s", err.Error())
			}
